...
const work: Position[] = [
  ...
  },
  {
    ...
    highlights: [
      'Acted as a technical lead in the autonomous team. Conducted technical requirements analysis, and architected the complete autonomous system framework, covering perception, vehicle modeling, state estimation, and control. Created documentations, visualizations, and presentations for the designed framework.',
      'Used Python 3 to build robust state estimation based on first-principle vehicle models and a Kalman filter to reconstruct all the race car states using sensor data (LiDAR, IMU, Gyroscope).',
      'Developed a new 2-DOF control system that generates control inputs through real-time optimization using Model Predictive Control (MPC) using Python 3 on ROS platform, eliminating instability issues caused by the decoupled PI and Stanley controllers. This significantly enhanced the system robustness and performance.',
      ...
      'Integrated systems via ROS and led cross-department system deployment and testing on the DUT24 race car. The built system was able to finish Skidpad racing robustly (90% of team failed).',
  ...
  },
  {
    ...
    highlights: [
      'Developed an automated data-processing pipeline in Python 3, enabling seamless data ingestion, NaN handling, restructuring, correlation analysis, and key feature extraction.',
      ...
      'Developed a fault detection module based on a clustering algorithm in Python 3 for anomaly detection and root cause analysis. Created visualization of the result using Power BI, enabling 3 times faster detection and providing interpretable error factors for debugging.',
